...
public class ANRequest<T extends ANRequest> {
    ...
    private HttpCallBackInChildThreadListener mHttpCallBackInChildThreadListener;
    ...
    public void getAsParsed(Type type, ParsedRequestListener parsedRequestListener) {
        this.mType = type;
        this.mResponseType = ResponseType.PARSED;
        this.mParsedRequestListener = parsedRequestListener;
        ANRequestQueue.getInstance().addRequest(this);
    ...
    public T setHttpCallBackInChildThreadListener(HttpCallBackInChildThreadListener httpCallBackInChildThreadListener) {
        this.mHttpCallBackInChildThreadListener = httpCallBackInChildThreadListener;
        return (T) this;
    }

    public HttpCallBackInChildThreadListener getHttpCallBackInChildThreadListener() {
        return mHttpCallBackInChildThreadListener;
    }
    ...
    public void updateDownloadCompletion() {
        ...
        if (mDownloadListener != null) {
            if (!isCancelled) {
                //add by liuzhi 20180823 for download md5 check begin
                if(mHttpCallBackInChildThreadListener != null){
                    mHttpCallBackInChildThreadListener.onCompletePreHandle(null);
                }
                //add by liuzhi 20180823 for download md5 check end
    ...
    public void deliverResponse(final ANResponse response) {
        try {
            ...
            if (!isCancelled) {
                ...
                //add by liuzhi 20180823 for download md5 check begin
                if(mHttpCallBackInChildThreadListener != null){
                    mHttpCallBackInChildThreadListener.onCompletePreHandle(response.getResult());
                }
                //add by liuzhi 20180823 for download md5 check end
