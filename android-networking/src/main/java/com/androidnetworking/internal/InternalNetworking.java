...
public final class InternalNetworking {
    ...
    public static Response performDownloadRequest(final ANRequest request) throws ANError {
        ...
        try {
            ...
            /*liuzhi 断点续传添加 20180815 begin*/
            // Support skipping:
            long preSaved = 0;
            String range = okHttpRequest.header("Range");
            if (range==null){
                range = okHttpRequest.header("range");
            }
            ...
            if (range != null) {
                if (range.startsWith("bytes=")) {
                    ...
                    try {
                        preSaved = Long.parseLong(range);
                    } catch (NumberFormatException nfe) {
                    }
                }
            }
            final long startFrom = preSaved;
            /*liuzhi 断点续传添加 20180815 end*/
                            ...
                            public Response intercept(Chain chain) throws IOException {
                                ...
                                return originalResponse.newBuilder()
                                        .body(new ResponseProgressBody(originalResponse.body(),
                                                request.getDownloadProgressListener(), startFrom))
                                        .build();
                            }
                        }).build();
            } else {
            ...
            final long startBytes = TrafficStats.getTotalRxBytes();
            okHttpResponse = request.getCall().execute();
            Utils.saveFile(okHttpResponse, request.getDirPath(), request.getFileName(), startFrom);
            final long timeTaken = System.currentTimeMillis() - startTime;
            if (okHttpResponse.cacheResponse() == null) {
                final long finalBytes = TrafficStats.getTotalRxBytes();
                final long diffBytes;
